      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteById(cart.skuId)">删除</a>
            <br />
            <a href="#none">收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      let res = await this.$store.dispatch("detail/updateShopCart", {
        skuId: cart.skuId,
        skuNum: disNum,
      });
      if (!res.code == 200) {
    ...
    async deleteById(skuId) {
      let res = await this.$store.dispatch("shopCart/deleteById", skuId);
      console.log(res);
      if (res.code == 200) {
        console.log("删除成功");
        this.getData();
    ...
    async deleteAllSelected() {
      try {
        let res = await this.$store.dispatch("shopCart/deleteAllSelected");
        this.getData();
      } catch (error) {
        alert(error.message);
      }
